/****** Object:  Procedure [dbo].[sp_3040_ProcessarHibrido]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE PROCEDURE [dbo].[sp_3040_ProcessarHibrido]
    @id_fundo       INT,
    @data           DATE,
    @user           VARCHAR(50),
    @cdError        INT OUTPUT,
...
BEGIN
    ...
    FROM TB_3040_BAS_FUNDO WHERE ID_FUNDO = @id_fundo
   ...
   --  Popular as tabelas de saída
    ...
    INNER JOIN  #AGREGAR                                 AS a    ON (o.DOC COLLATE DATABASE_DEFAULT = a.DOC_CLIENTE)
...
FROM TB_3040_XML_AGREG ) AS X ON X.ID_XML_AGREG = A.ID_XML_AGREG AND X.ID_XML = A.ID_XML
    ...
    GROUP BY    a20.CD_AUTORIZACAO,
				...
				O.DT_AQUISICAO,-- ADICIONADO EM 21/02/2018
                a16.CD_CLASS_RISCO_CLI,
                c.CONG_ECONOMICO,
                c.VL_FAT_ANUAL,
                c.DT_INI_RELAC,
   ...
   -- Atualiza o total de clientes
    ...
    FROM #OP as o
    inner join TB_3040_XML_CLI as c on (c.CD COLLATE DATABASE_DEFAULT = o.DOC COLLATE DATABASE_DEFAULT)

    ---------------------------------------------------------------------------------------------    
	CREATE TABLE #RECEBIVEIS_VS_ARQUIVO (ID_ARQUIVO INT,
	                                     DS_SEU_NUMERO VARCHAR(MAX))

	IF(@ID_FUNDO_CUSTODIA IS NOT NULL)
	BEGIN

		INSERT #RECEBIVEIS_VS_ARQUIVO
		SELECT ID_ARQUIVO, 
		       DS_SEU_NUMERO
		FROM FIDC_CUSTODIA_HML.dbo.TB_RECEBIVEL
		WHERE ID_FUNDO = @ID_FUNDO_CUSTODIA
	...
	BEGIN
		...
		SELECT ID_ARQUIVO, 
		       DS_SEU_NUMERO
		FROM FIDC_CUSTODIA_HML_2.dbo.TB_RECEBIVEL
		WHERE ID_FUNDO = @ID_FUNDO_CUSTODIA_2
		
	END

	UPDATE #OP SET ID_ARQUIVO = R.ID_ARQUIVO 
	FROM #OP O
	INNER JOIN #RECEBIVEIS_VS_ARQUIVO R ON(O.CD_CONTRATO COLLATE DATABASE_DEFAULT = R.DS_SEU_NUMERO COLLATE DATABASE_DEFAULT)
	--FIM: RUBEM TIROU NO DIA 20/02/2020, POIS ESTAVA MUITO LENTO NA BRL
    ...
    SELECT      IIF(O.CD_ANEXO_08 = '00', NULL,O.CD_ANEXO_08)           AS CARACESPECIAL, 					  	-- ALTERAÇÃO TIAGO 28/04/2020 
                C.CEP                                                   AS CEP,
                MAX(O.CD_ANEXO_17)                                      AS CLASSOP,
                MAX(O.CD_CONTRATO)                                      AS CONTRT,
                O.DS_CONTA_COSIF                                        AS COSIF,
                IIF(MAX(A11.TP_PESSOA) = 'J', MAX(C.DOC_CLIENTE), NULL) AS DETCLI,
                MAX(NULLIF(IIF(O.DT_VENCIMENTO < O.DT_POSICAO, ABS(DATEDIFF(DD, O.DT_POSICAO, O.DT_VENCIMENTO)), -1), -1)) AS DIAATRASO,
    ...
    GROUP BY    O.CD_ANEXO_08,
				...
				O.CD_ANEXO_06,
				O.ID_XML_CLI,
				O.DOC,-- ADICIONADO EM 21/02/2018
				O.TX_OPERACAO,
                O.DT_AQUISICAO,
				O.ID_ARQUIVO,
				C.CD_TP_PESSOA,
    ...
    INSERT INTO TB_3040_XML_OP
    SELECT      IIF(O.CD_ANEXO_08='00', NULL,O.CD_ANEXO_08)               AS CARACESPECIAL, 					  	
                C.CEP                                                     AS CEP,
                MAX(O.CD_ANEXO_17)                                        AS CLASSOP,
                MAX(O.CD_CONTRATO)                                        AS CONTRT,
                O.DS_CONTA_COSIF                                          AS COSIF,
    ...
    FROM #OP AS O
    INNER JOIN TB_3040_XML_OP AS C ON (C.CONTRT COLLATE DATABASE_DEFAULT = O.CD_CONTRATO COLLATE DATABASE_DEFAULT)
    ...
    SELECT      DISTINCT
                ...
                CASE    WHEN o.CD_ANEXO_02 = '04' AND 1=1 THEN o.VL_PERC_COOBRIGACAO
						...
						WHEN o.CD_ANEXO_02 = '05' THEN o.VL_PERC_COOBRIGACAO
    ...
    GROUP BY    o.DT_AQUISICAO, 
				...
				O.ID_FUNDO
    ---------------------------------------------------------------------------------------------
--	ALTERADO POR SOLICITACAO DE GISELE EM 17/04/2018
 ...
 --               o.ID_XML_OP
    ...
    INNER JOIN  TB_3040_BAS_GARANTIA                AS g    ON (    (   o.ID_OPERACAO IS NOT NULL
                                                                ...
                                                                OR  (   o.ID_OPERACAO IS NULL
                                                                    AND (g.CD_CONTRATO_CEDENTE COLLATE DATABASE_DEFAULT = o.CD_CONTRATO COLLATE DATABASE_DEFAULT)
                                                                    AND (G.CD_SISTEMA_ORIGEM COLLATE DATABASE_DEFAULT = o.CD_SISTEMA_ORIGEM COLLATE DATABASE_DEFAULT)
	...
	SELECT ID_XML_CLI,
		   @id_xml AS ID_XML
	into #cli
	FROM    TB_3040_XML_OP AS OP
	WHERE OP.ID_XML_CLI IN(SELECT ID_XML_CLI
							FROM TB_3040_XML_CLI AS CLI 
							WHERE CLI.ID_XML = @id_xml)
		...
		SELECT LEFT(CONVERT(VARCHAR,DtBase,120),7) AS DtBase,LEFT(CNPJ,8) AS CNPJ,Remessa,Parte,TpArq,NomeResp COLLATE SQL_LATIN1_GENERAL_CP1253_CI_AI AS NomeResp,EmailResp,TelResp,TotalCli,
			(   SELECT Cd,Tp,Autorzc,PorteCli,TpCtrl,LEFT(CONVERT(VARCHAR,IniRelactCli,120),10) AS IniRelactCli,FatAnual,CongEcon,ClassCli,
						(SELECT  DetCli,IPOC,Contrt,Mod,Cosif,OrigemRec,Indx,PercIndx,VarCamb,CEP,TaxEft,LEFT(CONVERT(VARCHAR,DtContr,120),10) AS DtContr,VlrContr,NatuOp,LEFT(CONVERT(VARCHAR,DtVencOp,120),10) AS DtVencOp,ClassOp,ProvConsttd,DiaAtraso,CaracEspecial,DtaProxParcela,VlrProxParcela,QtdParcelas,
							...
							(SELECT Tp,Cd,Cli.CD as Ident,Valor,CONVERT(NUMERIC(19,5),Perc) as Perc,Qtd FROM TB_3040_XML_INF AS Inf WHERE Inf.ID_XML_OP = Op.ID_XML_OP FOR XML AUTO, TYPE),
						  ...
						  FROM    TB_3040_XML_CLI AS Cli WHERE Cli.ID_XML = Doc3040.ID_XML and Cli.ID_XML_CLI in(SELECT ID_XML_CLI FROM #cli WHERE ID_XML = @id_xml)  FOR XML AUTO, TYPE),
